...
if __name__ == "__main__":
    ...
    parser.add_argument('--validation_qrels_gold', metavar='path', required=True,
                        help='the path to validation_qrels_gold')
    parser.add_argument('--artifacts_dumps_path', metavar='path', required=True,
                        help='the path to artifacts_dumps')
    parser.add_argument('--artifacts_summary_path', metavar='path', required=True,
                        help='the path to artifacts_summary')  # "artifacts/summary"
            ...
            for dump_file in files:
                ...
                if dump_file.endswith(".zip"):
                    with ZipFile(dump_file, "r") as zip_file:
                        eval_file = [f.filename for f in zip_file.filelist if "test" in f.filename]
                        print(eval_file)
    ...
    for model_dir, model_results in summary_dump.items():
        ...
        model_results_df.to_excel(
            str(os.path.join(args.artifacts_summary_path, os.path.split(model_dir)[-1])) + ".xlsx")
    ...
    performance_df.to_excel(os.path.join(args.artifacts_summary_path, f"ensemble_performance_df.{CUT_OFF}.xlsx"))
    pd.concat(all_results_df).to_excel(os.path.join(args.artifacts_summary_path, f"all_models_results.{CUT_OFF}.xlsx"))
